...
public class TugasBesar {
    ...
    static SQueue queue = new SQueue();

    static class Makanan {
        String nama;
        int harga;

        public Makanan(String _nama, int _harga) {
            this.nama = _nama;
            this.harga = _harga;
        }
    }
    ...
    static class Keranjang { // Makanan yang sudah dipilih pelanggan
        int[][] jumlah = new int[2][4]; // Jumlah makanan yang sudah dibeli jumlah {{a,b,c,d}, {a,b,c,d}}
        double totalKeranjang; // total harga pada keranjang
    }

    static class Client { // menu Pembeli
        String username;
        Keranjang keranjang;
    }

    static class TotalPenjualan {
    ...
    static class Admin {
        ...
        int penghasilan;
        TotalPenjualan total = new TotalPenjualan();
    }
    ...
    static boolean masuk() {
        ...
        if (counter != 5) {
            clients[counter].username = JOptionPane.showInputDialog(null, "Masukkan nama : ");
            ...
            switch (ask) {
                case JOptionPane.YES_OPTION:
                    ...
                    client.username = nama;
                    int queueIndex = queue.insert(client);
                    if (queue.search(nama) != -1) {
                        int userIndex = slotCheck();
                        if (userIndex != -1) {
                            clients[userIndex] = client;
                            menuUser(userIndex);

                            queue.clients[queueIndex] = null;
                        }
                    }
                    break;
    ...
    static boolean IsFull(SQueue queue) {
        if (queue.tail == queue.clients.length - 1) {
            return true;
        } else {
            return false;
        }
    ...
    static void waitingList() {
        if (IsFull(queue)) {
            JOptionPane.showMessageDialog(null, "Antrian Kenuh", "Queue Status", JOptionPane.ERROR_MESSAGE);
            menuUser();
        } else {
            Client client = new Client();
